# OneTab Plus - Chrome标签管理器 (v1.5.0)

这是一个Chrome浏览器扩展程序，用于高效管理和组织浏览器标签页。现已发布稳定的 1.5.0 版本。
...
- [优化同步策略](./docs/optimized-sync-strategy.md)
...
- **同步策略优化**：
  - 实现了优化的实时同步策略，移除了设备ID依赖
  - 添加了智能合并算法，提高了数据同步的效率和可靠性
  - 实现了单一用户通道，减少了服务器负担
  - 添加了精确更新机制，只更新变更的特定项
  - 添加了单个标签组同步功能，提高了同步效率
