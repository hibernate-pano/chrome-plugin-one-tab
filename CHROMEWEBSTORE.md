...
|---------|------|---------|--------|
| 1.17.3 | 2026-09-04 | 修复个别云端异常数据导致的加载崩溃 | Published（待确认） |
| 1.17.2 | 2026-09 | 同步稳定性修复 | Published（待确认） |
| 1.17.x（未发布） | 2026-09-08 | 同步层重构：单设备并发覆盖修复（保存后立即生效）、点开标签即时消失、上传延迟从 30s 降至 1.5~3s、列表渲染性能优化 | Draft |

## Review Notes

### Known Issues / Limitations
- 多设备同步要求登录；未登录用户仅使用本地功能。
- 云端同步删除的会话墓碑保留 30 天后清理（跨设备恢复窗口）。
