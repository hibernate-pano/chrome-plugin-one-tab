...
| Screenshot 1 [REQUIRED] | 1280×800 or 640×400 | ✅ Ready | store-assets/tapstack-main-1280x800.png |
| Screenshot 2 [RECOMMENDED] | 1280×800 or 640×400 | ✅ Ready | store-assets/tapstack-search-1280x800.png |
| Small Promo Tile [RECOMMENDED] | 440×280 | ✅ Ready | store-assets/tapstack-promo-440x280.png |
...
| `https://reccclnaxadbuccsrwmg.supabase.co/*` | host_permissions | 仅当用户登录 TapStack 云端账户时，用于登录鉴权与会话数据的跨设备同步。仅访问 TapStack 使用的固定 Supabase 实例，不访问其他网站。 |
...
|-----------|-----------|------------------------|---------|---------------------------|
| Personally identifiable info | Yes（登录邮箱） | Yes（仅传输到用户自己的云端账户） | 账户登录与数据归属 | No |
...
<!-- 隐私政策页面源码：src/web/public/privacy.html（最后更新 2026-09-13）。
     已作为 Vercel production alias 部署。 -->
...
|---------|------|---------|--------|
| 1.19.4 | 2026-09-13 | 发布安全收口：schema 探测失败不再误走硬删、删除失败保留重试、导入统一生成新 ID/印记并自动上云、服务端补齐设备平局规则；依赖审计归零 | 待提交 |
| 1.19.3 | 2026-09-13 | 同步数据安全修复：云端守卫改严格 `<`（标签删除/网页版写入不再被静默吞掉）、操作印记跨设备可比（换机/重装后可正常保存）、存量迁移接线；网页版登录持久化修复；导入数据现在会自动上云；依赖安全治理 | 未上架（仅 git tag） |
