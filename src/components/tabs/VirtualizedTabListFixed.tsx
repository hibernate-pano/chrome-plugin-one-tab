...
export const VirtualizedTabList: React.FC<VirtualizedTabListProps> = ({
  ...
  const calculateVisibleRange = useCallback(() => {
    ...
    setVisibleStartIndex(Math.max(0, startIndex - 5));
    setVisibleEndIndex(endIndex);
  }, [displayItems.length, itemHeight]);
  ...
  const renderVisibleItems = () => {
    const visibleItems = displayItems.slice(visibleStartIndex, visibleEndIndex);
    ...
    return visibleItems.map((item, index) => {
      ...
      return (
        <div key={`${item.type}-${(item.data as any).id}-${actualIndex}`} style={style}>
          {item.type === 'group' ? (
            <GroupHeader
...
const GroupHeader: React.FC<GroupHeaderProps> = ({ group, isExpanded, onToggle }) => {
  return (
    <div 
      className="group-header flex items-center p-3 bg-gray-50 border-b cursor-pointer hover:bg-gray-100 h-full"
      onClick={onToggle}
    >
...
const TabItem: React.FC<TabItemProps> = ({ 
  ...
  return (
    <div 
      className="tab-item flex items-center p-3 border-b hover:bg-gray-50 transition-colors h-full"
...
const MoveMenu: React.FC<MoveMenuProps> = ({ currentGroupId, groups, onMove, onClose }) => {
  ...
  return (
    <div className="move-menu absolute right-0 top-full mt-1 w-48 bg-white border rounded-md shadow-lg z-10">
      <div className="py-1">
