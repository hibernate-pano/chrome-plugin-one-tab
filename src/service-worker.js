// Chrome 扩展的 Service Worker (纯 JavaScript 版本)
// 为了避免模块导入和 TypeScript 编译问题，直接在Service Worker中实现核心功能
...
(function() {
  ...
  async function migrateStorageKeys() {
    try {
      const result1 = await chrome.storage.local.get(['tabGroups']);
      const result2 = await chrome.storage.local.get(['tab_groups']);
      const tabGroups = result1.tabGroups;
      const tab_groups = result2.tab_groups;
  ...
  const tabManager = {
    ...
    // 创建标签组
    createTabGroup: async function(tabs) {
      const validTabs = this.filterValidTabs(tabs);
      const now = new Date().toISOString();

      const formattedTabs = validTabs.map(function(tab) {
        return {
          id: generateId(),
    ...
    // 保存所有标签页
    saveAllTabs: async function(inputTabs) {
      ...
      try {
        ...
        await storage.setGroups([tabGroup].concat(existingGroups));
  ...
  chrome.runtime.onMessage.addListener(function(message, _sender, sendResponse) {
    ...
    try {
      switch (message.type) {
        ...
        case 'OPEN_TABS':
          if (Array.isArray(message.data && message.data.urls)) {
            Promise.all(
              message.data.urls.map(function(url) {
        ...
        case 'SAVE_ALL_TABS':
          ...
          (async function() {
            try {
              ...
              sendResponse({ success: false, error: (e && e.message) || '保存失败' });
            }
          })();
          return true; // 异步响应
