...
describe('syncMergeSafety: 同步合并不丢本地数据（真实生产路径防线）', () => {
  ...
  // ── 删除传播（tombstone 修复，2026-06-06 测试库端到端验证后固化）────────
  // 背景：markCloudGroupsAsDeleted 改为 update pending_delete=true（软删 tombstone），
  // downloadTabGroups 把列级 pending_delete OR 进 isDeleted。于是「另一台设备删了组」
  // 在本设备下载后会得到一个 isDeleted=true 的云端组，merge 必须据此删掉本地副本。
  // 旧行为是硬删 .delete()，云端行消失 → 本地副本被当 local-only 保留 → 删除不传播。
  it('删除传播：本地有组 + 云端同 id 标记 isDeleted（更高版本）→ merge 后删除', async () => {
    const { mergeTabGroups } = await import('@/utils/syncUtils');
    const local = [makeGroup('shared', 'Shared', { version: 1 })];
    // 云端 tombstone：同 id、isDeleted、版本更高（模拟另一设备删除后版本+1）
    const cloud = [makeGroup('shared', 'Shared', { isDeleted: true, version: 2 })];
    const merged = mergeTabGroups(local, cloud, 'newest');
    assert.equal(
      merged.some(g => g.id === 'shared' && !g.isDeleted),
      false,
      'tombstone（更高版本）必须把本地副本删除，否则删除不跨设备传播'
    );
  });

  it('删除传播：云端 tombstone 版本不高于本地 → 保留本地（不被旧删除信号误删）', async () => {
    const { mergeTabGroups } = await import('@/utils/syncUtils');
    const local = [makeGroup('shared', 'Shared', { version: 5 })];
    const cloud = [makeGroup('shared', 'Shared', { isDeleted: true, version: 2 })];
    const merged = mergeTabGroups(local, cloud, 'newest');
    assert.equal(
      merged.some(g => g.id === 'shared' && !g.isDeleted),
      true,
      '本地版本更高时不应被陈旧的云端删除信号删掉'
    );
  });
});
